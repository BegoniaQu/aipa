...
var AiPa = {
	...
	init: function(){
		...
		/*aipa_map.plugin('AMap.Geolocation', function () {
	    	...
	    	aipa_map.addControl(geolocation);
		...
		/*$.ajax({
	          ...
	          async: false,
	          dataType: "jsonp",
			  data:{tableid:"56dcf75a7bbf197f399d09be",limit:100,page:1,key:"192ad23a2f9b34977c1c363edb92ba80"},
	          jsonp: "callback",
	          jsonpCallback:"callback",
	          success: function(e){
	          	if (e.status) {	
	          		list_sellers  = e.datas;
	          		//添加商户信息
					AiPa.dataMake_sellers();
				}
	          }
	...
	//搜索标记点
	searchmaker: function(){
		var put = $(".C_commonInput"),
			btn = $(".C_buttn");

		btn.on('click',function(){
			if(put.val()!=""){
				$(".choise_city_box").hide();
				//根据id查询
				aipa_map.plugin('AMap.CloudDataSearch', function () {
					var search = new AMap.CloudDataSearch('541fb6b5e4b08b446bc31b9f');  
					search.searchById(put.val(), cloudSearch_CallBack);
				})
			}else{
				alert("请输入坐标码");
			}
		...
		function cloudSearch_CallBack(status, data){
			if (typeof data === "object" && !(data instanceof Array)){  
			    ...
			    if (hasProp){  
			       ...
			       aipa_map.setCenter(clouddata._location);
			       aipa_map.setZoom(19);
			       aipa_map.clearMap();
				   AiPa.dataMake();
				   console.log(clouddata);
				   //重新赋值
				   AiPa.markerVtemp(clouddata._id,clouddata.comment,clouddata.orientation,clouddata.position);

				   //给坐标详细信息传值
					AiPa.markerVdetail(clouddata);
			    }else{  
			       setTimeout(function(){
	...
	cloudDataTemp:function(){
		aipa_map.plugin('AMap.CloudDataLayer', function () {
			...
			ap_clouddatalayer = new AMap.CloudDataLayer("541fb6b5e4b08b446bc31b9f");
	...
	geoaddress: function(){
		var m;
		aipa_map.plugin(["AMap.Geocoder"], function() {        
			m = new AMap.Geocoder({ 
				radius: 1000,
				extensions: "all"
			});
			AMap.event.addListener(m, "complete", AiPa.geoaddress_html); 
			m.getAddress(aipa_map.getCenter()); 
		});
	},
	geoaddress_html: function(d){
		var a = d.regeocode.formattedAddress;
			$(".aipa-map-marker-number").html(a);
	},
	//商家坐标数据
	dataMake_sellers: function(){
		var t = list_sellers;
  		$.each(list_sellers,function(i,par){
  			AiPa.modelSellerMarker(t[i]);
  		})
	},
	//生成商户坐标
	modelSellerMarker: function(t){
		...
		var t = t,
			...
			//自定义坐标样式图层
			a = "<div class='q marker q3'><span></span></div>",
			//生成坐标源
			o = new AMap.Marker({
					map: aipa_map,
					content: a,
	...
	creatMake: function(){
		...
		go_btn.on('click',function(){
			AiPa.crtBoxtemp();
			$(".go_form").addClass("showSweetAlert");
			AiPa.crtBoxanm();
	...
	crtBoxtemp: function(){
		var shtml = '<!-- 创建标记信息 -->'+
					...
					'		<div class="go_item clearfix">'+
					'			<h4>说点什么吧</h4>'+
					'			<textarea id="comment" class="go_textarea"  placeholder="谈谈当时的感受，或留言"></textarea>'+
					'		</div>'+
					'	</div>'+
					'	<div class="go_btn">'+
